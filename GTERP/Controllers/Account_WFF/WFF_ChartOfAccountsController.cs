...
namespace GTERP.Controllers.Account_WFF
{
    ...
    public class WFF_ChartOfAccountsController : Controller
    {
        ...
        public WFF_ChartOfAccountsController(GTRDBContext _db, TransactionLogRepository tran)
        {
            tranlog = tran;
            db = _db;
            //Repository = rep;
        ...
        public ActionResult Index()
        {
            ...
            List<WFF_ChartOfAccount> chartofaccountlist = db.WFF_ChartOfAccounts.Take(1).Include(x => x.ParentChartOfAccount).Where(c => c.AccId > 0 && c.comid == comid && c.IsSysDefined == 0).ToList();

            return View(chartofaccountlist);
            //modelList = WFF_ChartOfAccount.prcGetData();
        ...
        public ActionResult COATree()
        {
            ...
            //List<WFF_ChartOfAccount> all = new List<WFF_ChartOfAccount>();
            //using (GTRDBContext db = new GTRDBContext())
            //{
            var all = db.WFF_ChartOfAccounts.Where(a => a.comid == comid).OrderBy(a => a.ParentID).ToList();
            //}
            return View(all);
        }
        ...
        public ActionResult Details(int Id)
        {
            ...
            //}
            //prcLoadCombo();
            //return View(WFF_ChartOfAccount.prcGetData(Id));
            return View();
        ...
        public ActionResult Create()
        {
            try
            {
                ...
                string comid = HttpContext.Session.GetString("comid");
                var model = new WFF_ChartOfAccount();


                var lastacccoa = db.WFF_ChartOfAccounts.Where(x => x.comid == comid).OrderByDescending(x => x.AccId).FirstOrDefault();
                ...
                //ViewBag.ParentIdId = new SelectList(db.WFF_ChartOfAccounts, "AccId", "AccName").ToList();

                //WFF_ChartOfAccount model = new WFF_ChartOfAccount();
                //ViewBag.ParentId = new SelectList(db.WFF_ChartOfAccounts, "AccId", "AccName").ToList();


                SqlParameter[] sqlParameter = new SqlParameter[1];
                sqlParameter[0] = new SqlParameter("@comid", comid);

                List<COAtemp> COAParent = Helper.ExecProcMapTList<COAtemp>("WFF_prcgetCOAParent", sqlParameter);
                ...
                //ViewBag.ParentId = db.WFF_ChartOfAccounts.Where(m => m.AccType == "G").ToList();
        ...
        public ActionResult Create(WFF_ChartOfAccount model, string title)
        {
                ...
                //if (ModelState.IsValid)
                {
                    if (model.AccId > 0)
                    {
                        ...
                        if (isModelBase == 1)
                        {
                            ...
                            {
                                var parentcodeinfo = db.WFF_ChartOfAccounts.Where(x => x.AccId == model.ParentID && x.comid == comid).FirstOrDefault();
                                string maxacccsubid = db.WFF_ChartOfAccounts.Where(x => x.ParentID == model.ParentID && x.comid == comid).Max(x => x.AccSubId).ToString();

                                if (model.AccType != model.PrevAccType)
                                {


                                    int ExistAccIdVoucher = db.WFF_VoucherSubs.Where(x => x.AccId == model.AccId).Count();
                                    int ExistAccIdSubAccountsHead = db.WFF_ChartOfAccounts.Where(x => x.ParentID == model.AccId && x.comid == comid).Count();
                                ...
                                if (maxacccsubid == null || maxacccsubid == "")
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        List<COAtemp> COAParent = Helper.ExecProcMapTList<COAtemp>("WFF_prcgetCOAParent", sqlParameter).ToList();
                            ...
                            if (title == "Delete")
                            {
                                ...
                                var count = db.WFF_ChartOfAccounts.Where(m => m.ParentID == model.AccId && m.comid == comid).ToList();
                        ...
                        else
                        {
                            ...
                            ////db.Database.ExecuteSqlCommand("prcTran_WFF_Account  @p0,@p1, @p2, @p3, @p4, @p5, @p6, @p7, @p8, @p9, @p10, @p11, @p12, @p13, @p14, @p15 ", parameters: new[]
                    ...
                    else
                    {
                        ...
                        if (isModelBase == 1)
                        {
                            ...
                            db.WFF_ChartOfAccounts.Add(model);
                            db.SaveChanges();
                    ...
                    //string sqlQuery = "Exec prcTran_WFF_Account 'NEW', " + Session["comid"] + ", 0, '" + model.AccName + "', '" + model.AccType + "', " + model.ParentID + ", " + double.Parse(model.OpDebit.ToString()) + ", " + double.Parse(model.OpCredit.ToString()) + ", 0, 0, 0 , " + model.CountryID + "," + double.Parse(model.Rate.ToString()) + ", " + model.opFYId.ToString() + ", '" + (model.OpDate) + "',0,0";
        ...
        public ActionResult Edit(int? Id)
        {
            ...
            WFF_ChartOfAccount chartofaccount = db.WFF_ChartOfAccounts.Find(Id);
        ...
        // [OverridableAuthorize]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(WFF_ChartOfAccount model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        public JsonResult DeleteConfirmed(int? id)
        {
            try
            {
                ...
                if (isModelBase == 1)
                {
                    ...
                    WFF_ChartOfAccount coa = db.WFF_ChartOfAccounts.Where(c => c.comid == (HttpContext.Session.GetString("comid")) && c.AccId == id).FirstOrDefault();

                    int ExistAccIdVoucher = db.WFF_VoucherSubs.Where(x => x.AccId == coa.AccId).Count();
                    int ExistAccIdSubAccountsHead = db.WFF_ChartOfAccounts.Where(x => x.ParentID == coa.AccId && x.comid == comid).Count();
                    ...
                    {
                        db.WFF_ChartOfAccounts.Remove(coa);
                        db.SaveChanges();
        ...
        public IActionResult Get()
        {
            try
            {
                ...
                var query = from e in db.WFF_ChartOfAccounts.Where(x => x.AccId > 0 && x.IsSysDefined == 0 && x.comid == comid).OrderByDescending(x => x.AccId)
        ...
        public string prcDataSave(WFF_ChartOfAccount model)
        {
        ...
        public string prcUpdateData(WFF_ChartOfAccount model)
        {
            //ArrayList arQuery = new ArrayList();
            //clsProcedure clsProc = new clsProcedure();
            //clsConnectionNew clsCon = new clsConnectionNew(Session["dbACC"].ToString(), true);
            try
            {
                ...
                //sqlQuery = "Update  tblWFF_COA Set Balance=" + dblBalance + ",IsSubsidiaryLedger=" + (((model.isSubsidiaryLedger)== true)? 1 : 0) + " where AccId=" + Int32.Parse(model.accId.ToString()) + " AND comid =  " + Session["comid"] + "";
                //arQuery.Add(sqlQuery);
        ...
        public string prcDeleteData(WFF_ChartOfAccount model)
        {
            //ArrayList arQuery = new ArrayList();
            //clsProcedure clsProc = new clsProcedure();
            //clsConnectionNew clsCon = new clsConnectionNew(Session["dbACC"].ToString(), true);
            try
            {
                ...
                //string sqlQuery = "Exec prcTran_WFF_Account 'DELETE', " + Session["comid"] + ",'" + model.accId + "' , '" + model.AccName + "', '" + ((model.isGroup == true) ? 'G' : 'L') + "', " + model.ParentID + ", " + double.Parse(model.OpDebit.ToString()) + ", " + double.Parse(model.OpCredit.ToString()) + ", 0, 0, 0 , " + model.CountryID + "," + double.Parse(model.Rate.ToString()) + ", " + model.opFYId.ToString() + ", '" + (model.opDate) + "'," + ((model.isSubsidiaryLedger == true) ? 1 : 0) + ",0";
                //arQuery.Add(sqlQuery);

                ////string  sqlQuery = "Exec prcTran_WFF_Account 'DELETE', " + Session["comid"] + ", " + model.accId + ",'" + model.AccCode + "', '', " + model.ParentID + ", 0, 0, 0, 0, 0,0, 0, 0, ''";
                ////arQuery.Add(sqlQuery);
        ...
        public JsonResult AccountsReport(string rptFormat, string action)
        {
            try
            {
                ...
                if (action == "PrintAccCOA")
                {
                    ...
                    var query = "Exec [WFF_rptCOA] '" + comid + "'";
                    HttpContext.Session.SetString("reportquery", "Exec [WFF_rptCOA] '" + comid + "'");
                ...
                //HttpContext.Session.SetObject("WFF_rptList", postData);
