    ...
    public class ProductsController : Controller
    {
        ...
        public JsonResult SetSessionProductReport(string rptFormat, string action, string ProductMainGroupId, string CategoryId, string AccIdInventory, string AccIdConsumption, string ProductId, string FromDate, string ToDate, string WarehouseId)
        {
            try
            {
                ...
                if (action == "PrintProductList")
                {
                    ...
                    HttpContext.Session.SetString("reportquery", "Exec Inv_rptProductList '" + comid + "','" + ProductMainGroupId + "', '" + CategoryId + "' ,'" + AccIdInventory + "' ,'" + AccIdConsumption + "','" + WarehouseId + "' , 0 ");
                ...
                {
                    //redirectUrl = new UrlHelper(Request.RequestContext).Action(action, "COM_BBLC_Master", new { FromDate = FromDate, ToDate = ToDate, Supplierid = SupplierId, CommercialCompanyId = CommercialCompanyId }); //, new { companyId = "7e96b930-a786-44dd-8576-052ce608e38f" }
                    reportname = "rptProductListwithQty";
                    filename = "Product_List_" + DateTime.Now.Date;
                    HttpContext.Session.SetString("reportquery", "Exec Inv_rptProductList '" + comid + "','" + ProductMainGroupId + "', '" + CategoryId + "' ,'" + AccIdInventory + "' ,'" + AccIdConsumption + "','" + WarehouseId + "' , 1 ");
                    HttpContext.Session.SetString("ReportPath", "~/ReportViewer/Inventory/" + reportname + ".rdlc");
                }
        ...
        public IActionResult Create()
        {
            ...
            if (userwiseproductlastdata != null)
            {
                ...
                ViewBag.CountryId = new SelectList(db.Countries, "CountryId", "CurrencyShortName", 18);
        ...
        //        var products = db.Products;//.Include(x=>x.vUnit).Include(x=>x.Category);
        //        var query = (from p in products

        //                     let costcallist = p.CostCalculated
        //                     .GroupBy(st => new { st.WarehouseId, st.ProductId })
        //                     .Select(grouping => grouping.Max(row => row.CostCalculatedId))
        //                     .ToArray()

        //                     let WarehouseQty =
        //                     //p.CostCalculated != null ?
        //                     p.CostCalculated.OrderByDescending(x => x.CostCalculatedId)
        //                     .Select(x => new WarehouseResult
        //                     {
        //                         CostCalculatedId = x.CostCalculatedId,
        //                         //WarehouseId = x.WarehouseId, 
        //                         WhShortName = x.Warehouses.WhShortName,
        //                         CurrentStock = x.CurrQty + x.PrevQty,
        //                         AverageCosting = x.CalculatedPrice,
        //                         CostingValue = (x.CalculatedPrice) * (x.CurrQty + x.PrevQty),
        //                         SalesValue = (decimal)p.SalePrice * (x.CurrQty + x.PrevQty)
        //                     })
        ...
        //                         CategoryName = p.vPrimaryCategory.Name,
        //                         ProductBarcode = p.ProductCode,
        ...
        public JsonResult Get()
        {
            try
            {
                ...
                var query = (from e in db.Products.Where(x => x.ProductId > 0 && x.comid == comid)//.Where(x => x.ProductId > 0 && x.comid == comid)//.OrderByDescending(x => x.ProductId)
                             ...
                             //////////////let costcallist = e.CostCalculated//.Where(x=>x.comid == comid && x.isDelete == false)
                             //////////////.GroupBy(st => new { st.WarehouseId, st.ProductId })
                             //////////////.Select(grouping => grouping.Max(row => row.CostCalculatedId))
                             //////////////.ToList() ?? null
                             ...
                             //let WarehouseQty =
                             ////p.CostCalculated != null ?
                             //    e.CostCalculated.OrderByDescending(x => x.CostCalculatedId)
                             //    .Select(x => new WarehouseResult
                             //    {
                             //        CostCalculatedId = x.CostCalculatedId,
                             //                                     //WarehouseId = x.WarehouseId, 
                             //        WhShortName = "",//x.Warehouses.WhShortName,
                             //        CurrentStock = 0,//x.CurrQty + x.PrevQty,
                             //        AverageCosting = 0,//x.CalculatedPrice,
                             //        CostingValue = 0,//(x.CalculatedPrice) * (x.CurrQty + x.PrevQty),
                             //        SalesValue = 0//(decimal)e.SalePrice * (x.CurrQty + x.PrevQty)
                             //    })
                             //    .Where(x => costcallist.Contains(x.CostCalculatedId))
                             //    //.Where(x => x.CostCalculatedId == 86)
                             //    .ToList()
                             ...
                             let WarehouseQty =
                                 //e.CostCalculated != null ?
                                 e.CostCalculated.Where(x => x.WarehouseId == 1).OrderByDescending(x => x.CostCalculatedId)
                                 .Select(x => new WarehouseResult
                                 {
                                     CostCalculatedId = x.CostCalculatedId,
                                     //WarehouseId = x.WarehouseId, 
                                     ...
                                     AverageCosting = x.CalculatedPrice,
                                     CostingValue = (x.CalculatedPrice) * (x.CurrQty + x.PrevQty),
                                     SalesValue = (decimal)e.SalePrice * (x.CurrQty + x.PrevQty)
                                 })
                                 //.Where(x => costcallist.Contains(x.CostCalculatedId))

                                 //.Where(b => costcallist.Any(i => i.Equals(b)))
                                 //.Where(x => costcallist.Contains(x.CostCalculatedId))
                                 //.Where(x => x.CostCalculatedId < 100000)
                                 .Take(1)
                                 ?? null
                             //: null
                             ...
                             select new ProductResult
                             {
                                 ...
                                 ImagePath = ImagePath,
                                 //ProductImage = null,//e.ProductImage != null ? Convert.ToBase64String(e.ProductImage) : null,//(asset.ProductImage != null) ? (asset.ProductImage) : null,
                                 ...
                                 //SingleCalculatedPrice = null//e.CostCalculated.OrderByDescending(x=>x.CostCalculatedId).Take(1).Select(x=>x.CalculatedPrice).FirstOrDefault()
                             });
