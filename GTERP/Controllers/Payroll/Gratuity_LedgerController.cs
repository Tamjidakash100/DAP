...
namespace GTERP.Controllers.Payroll
{
    ...
    public class Gratuity_LedgerController : Controller
    {
        ...
        public Gratuity_LedgerController(GTRDBContext context, clsProcedure _clsProc, TransactionLogRepository tran)//for report service
        {
        ...
        public ViewResult Index(string FromDate, string ToDate, string criteria)
        {
            ...
            if (FromDate == null || FromDate == "")
            {
                ...
                abcd = db.Gratuity_Ledgers.Include(x => x.vBankAccountNo).Where(x => x.comid == comid).ToList();
        ...
        public ActionResult Create()
        {
            ...
            var x = db.Gratuity_Ledgers.Where(x => x.comid == comid).OrderByDescending(x => x.GratuityLedgerId).FirstOrDefault();
            if (x != null)
            {
        ...
        public ActionResult Create(Gratuity_Ledger Gratuity_Ledger)
        {
            ...
            //if (ModelState.IsValid)
            {
                if (Gratuity_Ledger.GratuityLedgerId > 0)
                {
                    ...
                    Gratuity_Ledger.useridUpdate = userid;
                    //Gratuity_Ledger.isPost = true;


                    db.Entry(Gratuity_Ledger).State = EntityState.Modified;
                    db.SaveChanges();
                }
                else
                {
                    ...
                    db.Gratuity_Ledgers.Add(Gratuity_Ledger);
                    db.SaveChanges();


                    //db.Entry(Gratuity_Ledger).GetDatabaseValues();
                    //int id = Gratuity_Ledger.GovtScheduleId; // Yes it's here



                    //db.Categories.Add(Gratuity_Ledger);

                }
            }
            return RedirectToAction("Create");

            //return View(Gratuity_Ledger);
        }
        ...
        public ActionResult Edit(int? id)
        {
            ...
            Gratuity_Ledger Gratuity_Ledger = db.Gratuity_Ledgers.Where(c => c.GratuityLedgerId == id).FirstOrDefault();
            ...
            return View("Create", Gratuity_Ledger);
        ...
        public JsonResult SetSessionAccountReport(string rptFormat, string FromDate, string ToDate, int? BankAccId)
        {
            try
            {
                ...
                if (true)
                {
                    ...
                    query = "Exec Payroll_rptBankAccountLedger '" + comid + "', '" + FromDate + "' ,'" + ToDate + "' ,'" + BankAccId + "' , 'Gratuity Ledger'  ";
        ...
        public JsonResult DeleteConfirmed(int? id)
        {
            try
            {
                ...
                db.Gratuity_Ledgers.Remove(Gratuity_Ledger);
                db.SaveChanges();

                return Json(new { Success = 1, GovtScheduleId = Gratuity_Ledger.GratuityLedgerId, ex = "" });
