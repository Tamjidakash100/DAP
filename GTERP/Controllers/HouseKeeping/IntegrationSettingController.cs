    ...
    public class IntegrationSettingController : Controller
    {
        ...
        private void InitViewBag(string title, int? id = null, Cat_Integration_Setting_Main IntegrationSettingMian = null)
        {
            ...
            if (title == "Create")
            {
                ...
                ViewBag.AccId = new SelectList(db.Acc_ChartOfAccounts.Where(x => x.comid == comid && x.AccType == "L" && (x.AccCode.Contains("4-6-") || x.AccCode.Contains("4-1-") || x.AccCode.Contains("2-1-") || x.AccCode.Contains("1-1-11-") || x.AccCode.Contains("1-1-14-") || x.AccCode.Contains("1-1-28-"))).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
