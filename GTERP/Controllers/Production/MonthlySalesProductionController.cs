...
namespace GTERP.Controllers.HouseKeeping
{
    ...
    public class MonthlySalesProductionController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            return View(await db.MonthlySalesProductions.Include(p => p.YearName).Include(p => p.MonthName).Include(p => p.Unit).Include(x => x.ChartOfAccount).ToListAsync());
        }
        ...
        public IActionResult Create()
        {
            ...
            ViewBag.AccId = new SelectList(db.Acc_ChartOfAccounts.Where(x => x.comid == comid && x.AccCode.Contains("1-1-37") && x.AccType == "L"), "AccId", "AccName");


            var fiscalYear = db.Acc_FiscalYears.Where(f => f.isRunning == true && f.isWorking == true && f.comid == comid);
            if (fiscalYear != null)
            {
                ...
                ViewBag.FiscalMonthId = new SelectList(db.Acc_FiscalMonths.Where(y => y.FYId.ToString() == fiscalYear.FirstOrDefault().FYId.ToString()).OrderBy(x => x.FiscalMonthId), "FiscalMonthId", "MonthName");
                ViewBag.FiscalYearId = new SelectList(db.Acc_FiscalYears.Where(x => x.FiscalYearId == fiscalYear.FirstOrDefault().FiscalYearId).OrderBy(y => y.FiscalYearId), "FiscalYearId", "FYName", fiscalYear.FirstOrDefault().FiscalYearId);
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            ViewBag.FiscalMonthId = new SelectList(db.Acc_FiscalMonths.Where(y => y.ComId == comid && y.FYId == MonthlySalesProduction.YearName.FYId).OrderBy(x => x.FiscalMonthId), "FiscalMonthId", "MonthName", MonthlySalesProduction.FiscalMonthId);
            ViewBag.FiscalYearId = new SelectList(db.Acc_FiscalYears.Where(x => x.comid == comid).OrderBy(y => y.FiscalYearId), "FiscalYearId", "FYName", MonthlySalesProduction.FiscalYearId);
            ViewBag.AccId = new SelectList(db.Acc_ChartOfAccounts.Where(x => x.comid == comid && x.AccCode.Contains("1-1-37") && x.AccType == "L"), "AccId", "AccName", MonthlySalesProduction.AccId);
