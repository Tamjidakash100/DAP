...
namespace GTERP.Controllers.Buffer
{
    public class BufferWishBookingController : Controller
    {
        ...
        public async Task<IActionResult> Index(string FiscalYearId, string FiscalMonthId)
        {
            ...
            if (FiscalYearId == null)
            {
                ...
                if (countmax > 0)
                {
                    ...
                    listBuffertWiseBooking = _context.BuffertWiseBookings.Take(1).Include(d => d.BufferList).Include(d => d.YearName).Where(x => x.FiscalYearId == maxyear && x.FiscalMonthId == maxmonth && x.ComId == comid).ToList();
                ...
                else
                {
                    ...
                    ViewData["Representative"] = new SelectList(_context.BuferRepresentative.Where(x => x.BufferRepresentativeId > 0 && x.comid == comid), "BufferRepresentativeId", "RepresentativeName");
            ...
            {
                listBuffertWiseBooking = _context.BuffertWiseBookings.Take(1).Include(d => d.BufferList).Include(d => d.YearName).Where(x => x.FiscalYearId.ToString() == FiscalYearId && x.FiscalMonthId.ToString() == FiscalMonthId && x.ComId == comid).ToList();
            ...
            try
            {
                ...
                var query = from e in _context.BuffertWiseBookings.Where(x => x.BufferBookingId > 0 && x.ComId == comid).OrderByDescending(x => x.BufferBookingId)
                            select new BufferWiseBookingabc
                            {
                                ...
                                inputdate = e.DateAdded != null ? e.DateAdded.Value.ToString("d") : null,
        ...
        [HttpPost]
        public ActionResult GetAllBuffer(int? Year, int? Representative, string type)
        {
            ...
            parameters[2] = new SqlParameter("@Representative", Representative);
            parameters[3] = new SqlParameter("@AllotmentType", type);
        ...
        public IActionResult Create()
        {
            ...
            var lastbookingdata = _context.BuffertWiseBookings.Take(1).Where(x => x.ComId == comid).OrderByDescending(x => x.BufferList).FirstOrDefault();
        ...
        public IActionResult Create(List<BuffertWiseBooking> bufferWiseBooking)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    //var seldat=_context.BuffertWiseBookings.Where(s => s.BufferID==).Select(x=>x.DtInput)
                    DateTime date = (DateTime)item.DtInput;
        ...
        public async Task<IActionResult> Edit(BuffertWiseBooking bufferWiseBooking)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    DateTime date = (DateTime)bufferWiseBooking.DateAdded;
                    ...
                    //DateTime date = (DateTime)bufferWiseBooking.DtInput;
                    //string FiscalMonth = date.Year.ToString() + " (" + date.ToString("MMMM") + " )";

                    //var monthId = (from br in _context.Acc_FiscalMonths
                    //               where (br.MonthName == FiscalMonth)
                    //               select br.FiscalMonthId).FirstOrDefault();
                    //bufferWiseBooking.FiscalMonthId = monthId;
