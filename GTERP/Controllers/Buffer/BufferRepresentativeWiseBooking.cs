...
namespace GTERP.Controllers.Buffer
{
    ...
    public class BufferRepresentativeWiseBooking : Controller
    {
        ...
        public BufferRepresentativeWiseBooking(GTRDBContext context, TransactionLogRepository tran)
        {
            _context = context;
            tranlog = tran;
        ...
        public IActionResult DealerByDistricAndPolicStation(string AllotmentType, int? Yearid, int? Monthid, int? BufferListId, int? BufferRepresentativeId, int? fromcode, int? tocode, float? allotmentQty)
        {
            try
            {
                var result = "";
                ...
                var quary = $"EXEC PrcRepresentativeWiseBooking '{comid}',{Yearid},'{Monthid}','{BufferListId}','{BufferRepresentativeId}' ,'{allotmentQty}' ";
                ...
                parameters[4] = new SqlParameter("@RepreId", BufferRepresentativeId);
                parameters[5] = new SqlParameter("@allotmentQty", allotmentQty);
                //parameters[6] = new SqlParameter("@AllotmentType", AllotmentType);


                List<RepresentativeWiseDeliveryChallan> representativeWiseBooking = Helper.ExecProcMapTList<RepresentativeWiseDeliveryChallan>("PrcRepresentativeWiseBooking", parameters);

                return Json(new { representativeWiseBooking, ex = result });
            }
            catch (Exception ex)
            {
        ...
        public JsonResult AllotmentInfo(int id, int yearid, int monthid)
        {
            try
            {
                ...
                //if (PrevAllotmentQty != null)
                //{
                //    bufferbooking.PrevAllotmentQty = PrevAllotmentQty;
                //}
                //else
                //{
                //    bufferbooking.PrevAllotmentQty = 0.0;
                //}

                if (bufferbooking == null)
                {
        ...
        public class RepresentativeWiseDeliveryChallan
        {
        ...
        //Crete view DTO For Represantative
        public class bufferConnectionRepresentative
        {
            public int BufferRepresentativeId { get; set; }
            public string RepresentativeName { get; set; }
        }
        ...
        [HttpPost]
        public ActionResult GetPoliceStation(int BufferListId)
        {
            //List<Cat_PoliceStation> PoliceStation = _context.Cat_PoliceStation.Include(d => d.Cat_District).Where(p => p.DistId == distid).ToList();

            //List<RepresentativeBuffer> RepresentativeBuffer = _context.RepresentativeBuffers.Where(p => p.BufferListId == BufferListId).ToList();
            var GetRepresentative = (from p in _context.BuferRepresentative
                                     join s in _context.RepresentativeBuffers.Where(x => x.BufferListId == BufferListId) on p.BufferRepresentativeId equals s.BufferRepresentativeId
                                     select new bufferConnectionRepresentative
                                     {
                                         BufferRepresentativeId = s.BufferRepresentativeId,
            ...
            {
                foreach (bufferConnectionRepresentative item in GetRepresentative)
                {
                    items.Add(new SelectListItem { Value = item.BufferRepresentativeId.ToString(), Text = item.RepresentativeName });
                }
            }
            return Json(new SelectList(items, "Value", "Text"));
        }

        public ActionResult DistWiseBookingQtyCheck(string AllotmentType, int? BufferListId, string Year, string Month)
        {
            var comid = HttpContext.Session.GetString("comid");
            var BookingQty = _context.RepresentativeBooking.Where(b => b.ComId == comid && b.BufferListId == BufferListId && b.FiscalYearId.ToString() == Year && b.FiscalMonthId.ToString() == Month /*&& b.AllotmentType == AllotmentType*/).ToList();
            List<RepresentativeBooking> items = new List<RepresentativeBooking>();
            foreach (RepresentativeBooking item in BookingQty)
            {
                items.Add(item);
            }
            var Qty = items.Sum(q => q.AllotmentQty);
        ...
        //New Create Add
        public IActionResult RepCreate()
        {
