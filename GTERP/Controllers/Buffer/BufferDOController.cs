...
//        public IActionResult RepresentativeWiseBooking(int Repid) {

//            if (Repid <= 0)
//            {
//                return Json(new { flag = "0", msg = "invalid Buffer" });
//            }

//            var gf = (from info in _context.RepresentativeBooking.Where(c => c.BufferRepresentativeId == Repid)
                     
//                      select new BufferVm
//                      {
//                          RepresentativeBookingId = info.RepresentativeBookingId,
//                          BufferName = _context.Buffers.Where(w => w.BufferListId == info.BufferListId).Select(s => s.BufferName).FirstOrDefault() + "- Date:" + info.MonthName + "(" + info.DateAdded + ")",
//                          //BufferRepresentativeId = rbfrm.BufferRepresentativeId,
//                          //RepresentativeName = rbfrm.Name


//                      }).ToList();


//            var list = new SelectList(gf, "RepresentativeBookingId", "BufferName");

//            if (gf.Count > 0 && gf != null)
//            {
//                return Json(new { flag = "1", msg = "Buffer found", data = gf });
//            }
//            else
//            {
//                return Json(new { flag = "0", msg = "Buffer not found." });
//            }

//        }
//        public IActionResult RepresentativeBookingInfo(int BookId)
//        {
...
//            var prevOrder =_context.BufferDelOrder.Where(w => w.BufferRepresentativeId == BookId)
//                                                    .Select(s => s.RemainingQty).LastOrDefault();

//            var AllotmentDetails = (from info in _context.RepresentativeBooking.Where(c => c.BufferRepresentativeId == BookId)

//                                    select new BufferVm
//                                    {
//                                        RepresentativeBookingId = info.RepresentativeBookingId,
//                                        BufferName = _context.Buffers.Where(w => w.BufferListId == info.BufferListId).Select(s => s.BufferName).FirstOrDefault(),
//                                        AllotmentQty = info.AllotmentQty,
                                        
//                                        AvailabeQty= prevOrder<0?info.AllotmentQty: info.AllotmentQty - prevOrder


//                                    }).FirstOrDefault();


//            //RepresentativeBooking info =  _context.RepresentativeBooking.Where(c => c.BufferRepresentativeId == Repid).Select(s=>s).FirstOrDefault();

//            if (AllotmentDetails != null)
//            {
//                return Json(new { flag = "1", msg = "Buffer info found", info = AllotmentDetails,  });
//            }
//            else
//            {
//                return Json(new { flag = "0", msg = "Buffer info not found." });
//            }
...
//            ViewBag.RepresentativeId = new SelectList(_context.BuferRepresentative.Where(a => a.comid == comid), "BufferRepresentativeId", "RepresentativeName");
