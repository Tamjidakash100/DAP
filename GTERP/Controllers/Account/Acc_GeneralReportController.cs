...
namespace GTERP.Controllers.Account
{
    ...
    public class Acc_GeneralReportController : Controller
    {
        ...
        public ActionResult LedgerReport(string AccId, string FYId, string dtFrom, string dtTo, string CountryId, string IsLocalCurrency, string SupplierId, string CustomerId, string EmployeeId)
        {
            try
            {
        ...
        public ActionResult Report()
        {
            ...
            this.ViewBag.AccIdNoteOneCT = new SelectList(db.Acc_VoucherSubs
            //.Include(x => x.Acc_VoucherMain)
            .Where(p => p.Acc_VoucherMain.comid == comid && p.Note1.Contains("CT") && (p.Note1).Length > 3)
            ...
            int? SupplierId, int? CustomerId, int? EmployeeId, string AccIdNoteOneCT, string MinAccCode, string MaxAccCode)
        {
            ...
            try
            {
                ...
                if (rpttype == "CB")
                {
                    ...
                    //    HttpContext.Session.SetString("reportquery", "Exec Acc_rptCashBook '" + comid + "','" + userid + "', '" + dtFrom + "','" + dtTo + "', " + AccIdRecPay + ", " + Currency + " ");
