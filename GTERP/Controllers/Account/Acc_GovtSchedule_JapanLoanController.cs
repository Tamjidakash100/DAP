...
namespace GTERP.Controllers.Account
{
    ...
    public class Acc_GovtSchedule_JapanLoanController : Controller
    {
        ...
        public Acc_GovtSchedule_JapanLoanController(GTRDBContext context, clsProcedure _clsProc, TransactionLogRepository tran)//for report service
        {
        ...
        public ViewResult Index(string FromDate, string ToDate, string criteria)
        {
            ...
            if (FromDate == null || FromDate == "")
            {
                ...
                abcd = db.Acc_GovtSchedule_JapanLoans.Include(x => x.vAcc_ChartOfAccount).Where(x => x.comid == comid).ToList();
            ...
            //return View(db.Acc_GovtSchedule_JapanLoans.Where(c => c.GovtScheduleId > 0).ToList());
            return View(abcd);
        ...
        public ActionResult Create()
        {
            ...
            var x = db.Acc_GovtSchedule_JapanLoans.Where(x => x.comid == comid).OrderByDescending(x => x.GovtScheduleId).FirstOrDefault();
            if (x != null)
            {
        ...
        public ActionResult Create(Acc_GovtSchedule_JapanLoan Acc_GovtSchedulevar)
        {
            ...
            //if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    db.Acc_GovtSchedule_JapanLoans.Add(Acc_GovtSchedulevar);
                    db.SaveChanges();


                    //db.Entry(Acc_GovtSchedulevar).GetDatabaseValues();
                    //int id = Acc_GovtSchedule_JapanLoans.GovtScheduleId; // Yes it's here



                    //db.Categories.Add(Acc_GovtSchedule_JapanLoan);

                }
            }
            return RedirectToAction("Create");

            //return View(Acc_GovtSchedule_JapanLoan);
        }
        ...
        public ActionResult Edit(int? id)
        {
            ...
            Acc_GovtSchedule_JapanLoan Acc_GovtSchedule_JapanLoan = db.Acc_GovtSchedule_JapanLoans.Where(c => c.GovtScheduleId == id).FirstOrDefault();
            ...
            ViewData["Criteria"] = new SelectList(CriteriaList, "Value", "Text", Acc_GovtSchedule_JapanLoan.Criteria);
            var Acc_ChartOfAccount = db.Acc_ChartOfAccounts.Where(c => c.comid == comid && c.AccId > 0 && c.AccType == "L" && c.AccCode.Contains("2-2-10") || c.AccCode.Contains("2-2-11")); //&& c.ComId == (transactioncomid)
            this.ViewBag.AccId = new SelectList(Acc_ChartOfAccount.Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");

            ViewBag.Title = "Edit";

            return View("Create", Acc_GovtSchedule_JapanLoan);
        ...
        public JsonResult DeleteConfirmed(int? id)
        {
            try
            {
                ...
                db.Acc_GovtSchedule_JapanLoans.Remove(Acc_GovtSchedule_JapanLoan);
                db.SaveChanges();
