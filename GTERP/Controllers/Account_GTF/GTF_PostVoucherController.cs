...
namespace GTERP.Controllers.Account_GTF
{
    ...
    public class GTF_PostVoucherController : Controller
    {
        private TransactionLogRepository tranlog;
        Dictionary<int, dynamic> postData = new Dictionary<int, dynamic>();
        private GTRDBContext db;
        //public CommercialRepository Repository { get; set; } ///for report service
        public GTF_PostVoucherController(GTRDBContext _db, TransactionLogRepository tran)
        {
            db = _db;
        ...
        public ViewResult Index(string FromDate, string ToDate, string criteria)
        {
            ...
            List<GTF_VoucherMain> modellist = new List<GTF_VoucherMain>();
            ViewBag.Title = criteria;

            if (criteria == "All")
            {
                modellist = db.GTF_VoucherMains.Include(x => x.Acc_VoucherType).Where(p => p.comid == transactioncomid && (p.VoucherDate >= dtFrom && p.VoucherDate <= dtTo)).ToList();
            }
            else if (criteria == "Post")
            {
                modellist = db.GTF_VoucherMains.Include(x => x.Acc_VoucherType).Where(p => p.comid == transactioncomid && (p.VoucherDate >= dtFrom && p.VoucherDate <= dtTo) && p.isPosted == true).ToList();

            }
            else if (criteria == "UnPost")
            {
                modellist = db.GTF_VoucherMains.Include(x => x.Acc_VoucherType).Where(p => p.comid == transactioncomid && (p.VoucherDate >= dtFrom && p.VoucherDate <= dtTo) && p.isPosted == false).ToList();
            ...
            if (criteria.ToUpper().ToString() == "Post".ToUpper())
            {
                if (voucherid.Count() > 0)
                {
                    for (var i = 0; i < voucherid.Count(); i++)
                    {
                        ...
                        var singlevoucher = db.GTF_VoucherMains.Where(x => x.VoucherId == Convert.ToInt32(voucheridsingle)).FirstOrDefault();
        ...
        public string prcSaveData(GTF_VoucherMain model)
        {
