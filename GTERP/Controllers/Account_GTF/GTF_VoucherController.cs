...
namespace GTERP.Controllers.Account_GTF
{
    [OverridableAuthorize]
    public class GTF_VoucherController : Controller
    {
        ...
        public GTF_VoucherController(GTRDBContext context, clsProcedure _clsProc, TransactionLogRepository tran)//for report service
        {
            db = context;
        ...
        public ViewResult Index(string FromDate, string ToDate, string UserList)
        {
            ...
            //transactioncomid = "1";
            //var a = ;
            // return View(db.GTF_VoucherMains.Where(p => p.ComId == transactioncomid).ToList());
            ...
            if (UserList == null)
            {
                var X = db.GTF_VoucherMains.Include(x => x.Acc_VoucherType).Where(p => p.comid == transactioncomid && (p.VoucherDate >= dtFrom && p.VoucherDate <= dtTo)).ToList();
                return View(X);
            }
            else
            {
                if (UserList == "1")
                {
                    ...
                    {
                        X = db.GTF_VoucherMains.Include(x => x.Acc_VoucherType).Where(p => p.comid == transactioncomid).OrderByDescending(x => x.VoucherId).Take(5).ToList();
                        return View(X);
                ...
                {
                    var X = db.GTF_VoucherMains.Include(x => x.Acc_VoucherType).Where(p => p.comid == transactioncomid && (p.VoucherDate >= dtFrom && p.VoucherDate <= dtTo) && p.userid == UserList).ToList();
                    ...
                    {
                        X = db.GTF_VoucherMains.Include(x => x.Acc_VoucherType).Where(p => p.comid == transactioncomid && p.userid == UserList).OrderByDescending(x => x.VoucherId).Take(5).ToList();
                        return View(X);
        ...
        public ViewResult Details(int id)
        {
            ...
            GTF_VoucherMain Vouchermain = db.GTF_VoucherMains.Find(id);
        ...
        public ActionResult PrintCheck(int? id, string type)
        {
            ...
            var abcvouchermain = db.GTF_VoucherMains.Include(x => x.Acc_VoucherType).Where(x => x.VoucherId == id && x.comid == comid).FirstOrDefault();

            var reportname = "rptChk_janata";// db.GTF_VoucherMains.Where(x => x.VoucherId== id).Select(x => x.VoucherNo).FirstOrDefault();
            ...
            var str = db.GTF_VoucherMains.Include(x => x.Acc_VoucherType).FirstOrDefault().Acc_VoucherType.VoucherTypeNameShort;// "VPC";
            //var Currency = "18";
            HttpContext.Session.SetString("reportquery", "Exec acc_rptCheckPrint  '" + id + "','" + comid + "' , 'ChequeNo'");

            string filename = db.GTF_VoucherSubChecnos.Where(x => x.VoucherId == id).Select(x => x.ChkNo).FirstOrDefault();
        ...
        public ActionResult Print(int? id, string type)
        {
            ...
            var reportname = "rptShowVoucher";// db.GTF_VoucherMains.Where(x => x.VoucherId== id).Select(x => x.VoucherNo).FirstOrDefault();
            ...
            //int WarehouseCount = db.GTF_VoucherMains.Where(x => x.VoucherId == id).Count(); 
            //if (WarehouseCount > 0) { reportname = "rptShowVoucher_SubRpt"; }

            HttpContext.Session.SetString("ReportPath", "~/ReportViewer/Accounts/" + reportname + ".rdlc");
            var str = db.GTF_VoucherMains.Include(x => x.Acc_VoucherType).FirstOrDefault().Acc_VoucherType.VoucherTypeNameShort;// "VPC";
            var Currency = "18";
            HttpContext.Session.SetString("reportquery", "Exec GTF_rptVoucher 0, 'VID','All', '" + comid + "' , '01-Jan-1900', '01-Jan-1900', '" + str + "','" + str + "', " + id + ", " + Currency + ", 0");
            ...
            string filename = db.GTF_VoucherMains.Where(x => x.VoucherId == id).Select(x => x.VoucherNo + "_" + x.Acc_VoucherType.VoucherTypeName).Single();
            HttpContext.Session.SetString("PrintFileName", filename.Replace(".", "").Replace(" ", "").Replace(",", "").Replace("\"", ""));
            ...
            subReport.strRFNSub = "";
            subReport.strQuerySub = "Exec [rptShowVoucher_Referance] '" + id + "','" + comid + "','ChequeNo_GTF'";
            ...
            subReport.strDSNSub = "DataSet1";
            subReport.strRFNSub = "";
            subReport.strQuerySub = "Exec [rptShowVoucher_Referance] '" + id + "','" + comid + "','ReceiptPerson_GTF'";
        ...
        public ActionResult Create(string Type, int VoucherId = 0, int FiscalMonthId = 0, int? IntegrationSettingMainId = 0)
        {
            try
            {
                ...
                //ViewBag.Title = "Entry";
                var transactioncomid = HttpContext.Session.GetString("comid");
                var lastvoucher = db.GTF_VoucherMains.Where(x => x.Acc_VoucherType.VoucherTypeNameShort == Type && x.comid == transactioncomid).OrderByDescending(x => x.VoucherId).FirstOrDefault();
                ...
                GTF_VoucherMain vouchersamplemodel = new GTF_VoucherMain();
                ...
                //GTF_VoucherSub VoucherSubs = new GTF_VoucherSub();
                //VoucherSubs.VoucherSubSections = new List<GTF_VoucherSubSection>();
                //VoucherSubs.VoucherSubChecnoes = new List<GTF_VoucherSubChecno>();
                ...
                var ChartOfAccountParent = db.GTF_ChartOfAccounts.Where(c => c.AccId > 0 && c.AccType == "G" && c.comid == transactioncomid).Select(s => new { Text = s.AccName, Value = s.AccId }).ToList(); //&& c.ComId == (transactioncomid)
                ...
                if (Type == "VPC")
                {
                    ...
                    var GTF_ChartOfAccount = db.GTF_ChartOfAccounts.Where(c => c.comid == transactioncomid && c.AccId > 0 && c.AccType == "L"); //&& c.ComId == (transactioncomid)
                    if (transactioncompany.isMultiDebitCredit == true)
                    {
                        this.ViewBag.Account = new SelectList(GTF_ChartOfAccount.Where(x => x.IsBankItem == false).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    }
                    else
                    {
                        this.ViewBag.Account = new SelectList(GTF_ChartOfAccount.Where(x => x.IsBankItem == false && x.IsCashItem == false).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    }
                    ...
                    //this.ViewBag.AccountSearch = GTF_ChartOfAccount;
                    ...
                    if (VoucherId == 0)
                    {
                        ...
                        this.ViewBag.AccountMain = new SelectList(db.GTF_ChartOfAccounts.Where(p => p.comid == transactioncomid && p.AccType == "L" && p.IsCashItem == true).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    ...
                    else
                    {
                        ...
                        GTF_VoucherMain Vouchermain = db.GTF_VoucherMains
                            .Include(b => b.VoucherSubs).ThenInclude(b => b.GTF_ChartOfAccount).ThenInclude(b => b.ParentChartOfAccount)
                        ...
                        this.ViewBag.AccountMain = new SelectList(db.GTF_ChartOfAccounts.Where(p => p.comid == transactioncomid && p.AccType == "L" && p.IsCashItem == true).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text", AccId);
                ...
                else if (Type == "VRB")
                {
                    ...
                    var GTF_ChartOfAccount = db.GTF_ChartOfAccounts.Where(c => c.comid == transactioncomid && c.AccId > 0 && c.AccType == "L" && c.IsItemAccmulateddDep == false && c.IsItemDepExp == false); //&& c.ComId == (transactioncomid)
                    if (transactioncompany.isMultiDebitCredit == true)
                    {
                        this.ViewBag.Account = new SelectList(GTF_ChartOfAccount.Where(x => x.IsCashItem == false).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    }
                    else
                    {
                        this.ViewBag.Account = new SelectList(GTF_ChartOfAccount.Where(x => x.IsBankItem == false && x.IsCashItem == false).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    ...
                    ///////only cash item when multi debit credit of then it enable
                    this.ViewBag.AccountMain = new SelectList(db.GTF_ChartOfAccounts.Where(p => p.comid == transactioncomid && p.AccType == "L" && p.IsBankItem == true).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    this.ViewBag.SubSectionList = db.Cat_SubSection.Where(x => x.ComId == transactioncomid);
                    ...
                    else
                    {
                        ...
                        this.ViewBag.Country = new SelectList(db.Countries.Where(x => x.isActive == true), "CountryId", "CurrencyShortName", Vouchermain.CountryId);
                        int AccId = Vouchermain.VoucherSubs.Where(x => x.SRowNo < 0).Select(x => x.AccId).FirstOrDefault();
                        this.ViewBag.AccountMain = new SelectList(db.GTF_ChartOfAccounts.Where(p => p.comid == transactioncomid && p.AccType == "L" && p.IsBankItem == true).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text", AccId);
                    ...
                    if (VoucherId == 0)
                    {
                        ...
                        try
                        {
                            ...
                            if (FiscalMonthId > 0 && IntegrationSettingMainId > 0)
                            {
                                ...
                                var abcdefgh = db.Cat_GTFIntegrationSummary.Include(x => x.GTF_ChartOfAccounts).ThenInclude(x => x.ParentChartOfAccount).Where(x => x.FiscalMonthId == FiscalMonthId && x.DataType == IntegrationSettingMainId.ToString() && x.GTF_ChartOfAccounts.AccType == "L").ToList(); //&& (x.TKCreditLocal + x.TKCreditLocal) > 0
                                ...
                                vouchersamplemodel.VoucherSubs = new List<GTF_VoucherSub>();
                                foreach (var item in abcdefgh)
                                {
                                    if ((item.TKDebitLocal + item.TKCreditLocal) > 0)
                                    {
                                        GTF_VoucherSub abc = new GTF_VoucherSub();
                                        ...
                                        abc.GTF_ChartOfAccount = item.GTF_ChartOfAccounts;
                ...
                else if (Type == "VCR")
                {
                    ...
                    if (transactioncompany.isMultiDebitCredit == true)
                    {
                        this.ViewBag.Account = new SelectList(GTF_ChartOfAccount.Where(x => x.IsBankItem == true || x.IsCashItem == true).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    }
                    else
                    {
                        this.ViewBag.Account = new SelectList(GTF_ChartOfAccount.Where(x => x.IsBankItem == true || x.IsCashItem == true).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    ...
                    ///////only cash item when multi debit credit of then it enable
                    this.ViewBag.AccountMain = new SelectList(db.GTF_ChartOfAccounts.Where(p => p.comid == transactioncomid && p.AccType == "L" && p.IsBankItem == true || p.IsCashItem == true).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    this.ViewBag.SubSectionList = db.Cat_SubSection.Where(x => x.ComId == transactioncomid);
                    ...
                    else
                    {
                        ...
                        this.ViewBag.Country = new SelectList(db.Countries.Where(x => x.isActive == true), "CountryId", "CurrencyShortName", Vouchermain.CountryId);
                        int AccId = Vouchermain.VoucherSubs.Where(x => x.SRowNo < 0).Select(x => x.AccId).FirstOrDefault();
                        this.ViewBag.AccountMain = new SelectList(db.GTF_ChartOfAccounts.Where(p => p.comid == transactioncomid && p.AccType == "L" && p.IsCashItem == true || p.IsBankItem == true).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text", AccId);
                ...
                else if (Type == "VJR")
                {
                    ...
                    if (transactioncompany.isMultiDebitCredit == true)
                    {
                        this.ViewBag.Account = new SelectList(GTF_ChartOfAccount.Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    }
                    else
                    {
                        this.ViewBag.Account = new SelectList(GTF_ChartOfAccount.Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    ...
                    ///////only cash item when multi debit credit of then it enable
                    this.ViewBag.AccountMain = new SelectList(db.GTF_ChartOfAccounts.Where(p => p.comid == transactioncomid && p.AccType == "L").Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");
                    this.ViewBag.SubSectionList = db.Cat_SubSection.Where(x => x.ComId == transactioncomid);
                    ...
                    else
                    {
                        ...
                        this.ViewBag.Country = new SelectList(db.Countries.Where(x => x.isActive == true), "CountryId", "CurrencyShortName", Vouchermain.CountryId);
                        int AccId = Vouchermain.VoucherSubs.Where(x => x.SRowNo < 0).Select(x => x.AccId).FirstOrDefault();
                        this.ViewBag.AccountMain = new SelectList(db.GTF_ChartOfAccounts.Where(p => p.comid == transactioncomid && p.AccType == "L").Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text", AccId);
        ...
        {
            //public IEnumerable<GTF_ChartOfAccount> coa { get; set; }
        ...
        public class GTF_ChartOfAccount_view
        {
            public int AccId { get; set; }
        ...
        public JsonResult AccountInfo(int id)
        {
            try
            {
                ...
                //GTF_ChartOfAccount chartofaccount = new GTF_ChartOfAccount();
                ...
                GTF_ChartOfAccount_view abc = new GTF_ChartOfAccount_view();
                GTF_ChartOfAccount chartofaccount = db.GTF_ChartOfAccounts.Where(y => y.AccId == id).SingleOrDefault();
        ...
        //public JsonResult Create(GTF_VoucherMain GTF_voucherMain)
        public IActionResult Create(GTF_VoucherMain GTF_voucherMain/*, List<HR_Loan_Data_HouseBuilding> details*/)
        {
            try
            {
                ...
                .Where(p => p.LockType.Contains("Account Lock") && p.DtDate.Date <= GTF_voucherMain.VoucherDate.Date && p.DtToDate.Value.Date >= GTF_voucherMain.VoucherDate.Date
                ...
                GTF_voucherMain.VAmount = double.Parse(clsProc.GTRFormatCurrencyBDT(GTF_voucherMain.VAmount.ToString()));
                GTF_voucherMain.vAmountInWords = clsProc.GTRInwordsFormatBD(GTF_voucherMain.VAmount.ToString(), "", "");
                GTF_voucherMain.DateUpdated = DateTime.Now.Date;
                GTF_voucherMain.useridUpdate = userid;


                if (GTF_voucherMain.VoucherTypeId == 3)
                {
                    var abcsum = 0.00;
                    var bankacccode = "";
                    foreach (var item in GTF_voucherMain.VoucherSubs)
                    {

                        bankacccode = db.GTF_ChartOfAccounts.Where(p => p.comid == comid && p.AccType == "L" && p.IsBankItem == true && p.AccId == item.AccId).Select(x => x.AccCode).FirstOrDefault();
                        if (bankacccode != null)
                        {
                    ...
                    //GTF_voucherMain.VoucherSubs = 
                    //var inwordsfigure = GTF_voucherMain.VoucherSubs.Where(x => x.GTF_ChartOfAccount.AccCode.Contains("1-1-11")).Sum(x => x.TKCredit);
                    GTF_voucherMain.vAmountInWords = clsProc.GTRInwordsFormatBD(abcsum.ToString(), "", "");
                }
                ...
                //    string objct = d["GTF_VoucherMain"].ToString();
                //    GTF_VoucherMain model = JsonConvert.DeserializeObject<GTF_VoucherMain>(objct);


                //    GTF_VoucherMain GTF_voucherMain = model;


                GTF_voucherMain.VoucherInputDate = DateTime.Now.Date;
                ...
                {
                    //var monthid = GTF_voucherMain.VoucherDate.Month;// && x.dtFrom.ToString() == monthid.ToString()

                    //var monthlist = 

                    DateTime date = GTF_voucherMain.VoucherInputDate;
                    ...
                    // Perform Update
                    if (GTF_voucherMain.VoucherId > 0)
                    {

                        ViewBag.Title = "Edit";

                        //var CurrentProductSerial = db.ProductSerial.Where(p => p.VoucherId == GTF_voucherMain.VoucherId);
                        var CurrentVoucherSub = db.GTF_VoucherSubs.Include(x => x.VoucherSubChecnoes).Include(x => x.VoucherSubSections).Where(p => p.VoucherId == GTF_voucherMain.VoucherId);
                        var CurrentVoucherSubcheckno = db.GTF_VoucherSubChecnos.Where(p => p.VoucherId == GTF_voucherMain.VoucherId);

                        //var CurrentVoucherSubSection = db.GTF_VoucherSubSections.Where(p => p.VoucherId == GTF_voucherMain.VoucherId);
                        //var CurrentVoucherCheck = db.GTF_VoucherSubChecnos.Where(p => p.VoucherId == GTF_voucherMain.VoucherId);
                        GTF_voucherMain.useridUpdate = userid;
                        GTF_voucherMain.DateUpdated = DateTime.Now;
                        ...
                        //db.GTF_VoucherSubs.Remove(CurrentVoucherSub);
                        //db.SaveChanges();

                        //foreach (GTF_VoucherSub ss in CurrentVoucherSub)
                        //{
                        //    //    foreach (GTF_VoucherSubCheckno sss in ss.VoucherSubChecnoes)
                        //    //    db.GTF_VoucherSubChecnos.Remove(sss);

                        //    //    foreach (GTF_VoucherSubSection ssss in ss.VoucherSubSections)
                        //    //    db.GTF_VoucherSubSections.Remove(ssss);


                        //     db.GTF_VoucherSubs.Remove(ss);

                        //}


                        db.GTF_VoucherSubChecnos.RemoveRange(CurrentVoucherSubcheckno);
                        db.GTF_VoucherSubs.RemoveRange(CurrentVoucherSub);
                        db.SaveChanges();

                        //foreach (GTF_VoucherSubCheckno ss in CurrentVoucherCheck)
                        //    db.GTF_VoucherSubs.Remove(ss);
                        //db.SaveChanges();


                        foreach (GTF_VoucherSub ss in GTF_voucherMain.VoucherSubs)
                        {
                            //if (ss.VoucherSubId > 0)
                            //{
                            //db.Entry(ss).State = EntityState.Modified;
                            ss.VoucherSubId = 0;
                            db.GTF_VoucherSubs.Add(ss);
                            //db.SaveChanges();
                            ...
                            //foreach (GTF_VoucherSubSection sss in ss.VoucherSubSections)
                            //{
                            //    sss.VoucherSubId = ss.VoucherSubId;
                            //    db.GTF_VoucherSubSections.Add(sss);
                            ...
                            //foreach (GTF_VoucherSubCheckno ssss in ss.VoucherSubChecnoes)
                            //{
                            //    ssss.VoucherSubId = ss.VoucherSubId;
                            //    db.GTF_VoucherSubChecnos.Add(ssss);

                            //}

                        }
                        db.SaveChanges();
                        //foreach (GTF_VoucherSubSection sss in CurrentVoucherSubSection)
                        //db.GTF_VoucherSubSections.Remove(sss);


                        //foreach (GTF_VoucherSubChecno ssss in CurrentVoucherCheck)
                        //db.GTF_VoucherSubChecnos.Remove(ssss);


                        //if (GTF_voucherMain.VoucherSubs == null)
                        //{
                        //}
                        //else
                        //{
                        //    foreach (GTF_VoucherSubSection subsection in GTF_voucherMain.VoucherSubs.)
                        //    db.GTF_VoucherSubSections.Add(subsection);
                        //}
                        ...
                        //    //foreach (GTF_VoucherSubChecno checkno in GTF_voucherMain.vVouchersubMain.VoucherSubChecnos)
                        //    //    db.GTF_VoucherSubChecnos.Add(checkno);
                        //}
                        GTF_voucherMain.FiscalMonthId = activefiscalmonth.FiscalMonthId;
                        GTF_voucherMain.FiscalYearId = activefiscalyear.FiscalYearId;


                        db.Entry(GTF_voucherMain).State = EntityState.Modified;



                        TempData["Message"] = "Data Update Successfully";
                        TempData["Status"] = "2";
                        tranlog.TransactionLog(RouteData.Values["controller"].ToString(), RouteData.Values["action"].ToString(), TempData["Message"].ToString(), GTF_voucherMain.VoucherId.ToString(), "Update", GTF_voucherMain.VoucherNo.ToString());
                    ...
                    else
                    {
                        ...
                        GTF_voucherMain.userid = userid;// HttpContext.Session.GetString("userid");
                        GTF_voucherMain.comid = comid;
                        GTF_voucherMain.DateAdded = DateTime.Now;
                        GTF_voucherMain.FiscalMonthId = activefiscalmonth.FiscalMonthId;
                        GTF_voucherMain.FiscalYearId = activefiscalyear.FiscalYearId;


                        //var voucherfiscalyear = db.Acc_FiscalYears.Where(x => x.OpeningDate >= GTF_voucherMain.VoucherDate).FirstOrDefault();
                        //var voucherfiscalyear = db.Acc_FiscalYears.Where(x => x.OpeningDate >= GTF_voucherMain.VoucherDate && x.ClosingDate <= GTF_voucherMain.VoucherDate).FirstOrDefault();
                        var voucherfiscalyear = db.Acc_FiscalYears.Where(x => x.comid == comid && x.OpeningDate <= GTF_voucherMain.VoucherDate && x.ClosingDate >= GTF_voucherMain.VoucherDate).FirstOrDefault();// && x.dtFrom.ToString() == monthid.ToString()

                        ////voucherfiscalyear.FiscalYearId,
                        ///
                        var x = VoucherNoMaker(GTF_voucherMain.comid, GTF_voucherMain.VoucherTypeId, GTF_voucherMain.VoucherDate, activefiscalyear.FiscalYearId, activefiscalmonth.FiscalMonthId); // nned to work.. //// activefiscalyear.FiscalYearId 
                        GTF_voucherMain.VoucherNo = x[0];
                        GTF_voucherMain.VoucherSerialId = int.Parse(x[1]);

                        db.GTF_VoucherMains.Add(GTF_voucherMain);

                        db.SaveChanges();

                        db.Entry(GTF_voucherMain).GetDatabaseValues();
                        int id = GTF_voucherMain.VoucherId; // Yes it's here

                        var abc = GTF_voucherMain.VoucherSubs.ToList();
                    ...
                    TempData["Status"] = "1";
                    tranlog.TransactionLog(RouteData.Values["controller"].ToString(), RouteData.Values["action"].ToString(), TempData["Message"].ToString(), GTF_voucherMain.VoucherId.ToString(), "Create", GTF_voucherMain.VoucherNo.ToString());


                    // If Sucess== 1 then Save/Update Successfull else there it has Exception
                }
                return Json(new { Success = 1, VoucherID = GTF_voucherMain.VoucherId, ex = "" });
        ...
        public string[] VoucherNoMaker(string comid, int vouchertypeid, DateTime voucherdate, int fiscalyearid, int fiscalmonthid)
        {
            ...
            if (voucernocreatestyle == "LifeTime")
            {
                ...
                maxvoucherid = db.GTF_VoucherMains.Where(x => x.comid == comid && x.VoucherTypeId == vouchertypeid).Max(x => x.VoucherSerialId);
            ...
            else if (voucernocreatestyle == "Yearly")
            {
                ...
                maxvoucherid = db.GTF_VoucherMains.Where(x => x.comid == comid && x.VoucherTypeId == vouchertypeid && x.FiscalYearId == fiscalyearid).DefaultIfEmpty().Max(p => p == null ? 0 : p.VoucherSerialId);
                input = maxvoucherid + 1;
                maxnowithpadleftresult = input.ToString().PadLeft(length, '0');
            ...
            else if (voucernocreatestyle == "Monthly")
            {
                ...
                maxvoucherid = (db.GTF_VoucherMains.Where(x => x.comid == comid && x.VoucherTypeId == vouchertypeid && x.FiscalMonthId == fiscalmonthid).Max(x => (int?)x.VoucherSerialId)) ?? 0;
        ...
        public ActionResult Edit(string Type, int? VoucherId)
        {
            ...
            try
            {
                ...
                ViewBag.PrdUnitId = new SelectList(db.PrdUnits.Where(c => c.comid == transactioncomid), "PrdUnitId", "PrdUnitName"); //&& c.ComId == (transactioncomid)
                                                                                                                                     ///////account head parent data for dropdown
                var ChartOfAccountParent = db.GTF_ChartOfAccounts.Where(p => p.comid == transactioncomid).Where(c => c.AccId > 0 && c.AccType == "G" && c.comid == transactioncomid).Select(s => new { Text = s.AccName, Value = s.AccId }).ToList(); //&& c.ComId == (transactioncomid)
                this.ViewBag.AccountParent = new SelectList(ChartOfAccountParent, "Value", "Text");
                ...
                //var GTF_ChartOfAccount = db.GTF_ChartOfAccounts.Where(c => c.AccId > 0 && c.AccType == "L"); //&& c.ComId == (transactioncomid)
                //this.ViewBag.Account = new SelectList(GTF_ChartOfAccount, "AccId", "AccName");
                ////this.ViewBag.AccountSearch = GTF_ChartOfAccount;

                //List<VoucherChartOfAccount> COAParent = (db.Database.SqlQuery<VoucherChartOfAccount>("[prcGetVoucherAccounts]  @comid,@Type,@dtvoucher", new SqlParameter("comid",HttpContext.Session.GetString("comid");), new SqlParameter("Type", Type), new SqlParameter("dtvoucher", DateTime.Now.Date.ToString("dd-MMM-yy")))).ToList();
                //this.ViewBag.AccountSearch = COAParent;

                //var ChartOfAccountParent = db.GTF_ChartOfAccounts.Where(c => c.AccId > 0 && c.AccType == "G"); //&& c.ComId == (transactioncomid)
        ...
        public ActionResult Delete(int? VoucherId)
        {
            try
            {
               ...
               .Where(x => x.VoucherId == VoucherId).FirstOrDefault();
                ...
                ///////only cash item when multi debit credit of then it enable
                this.ViewBag.AccountMain = new SelectList(db.GTF_ChartOfAccounts.Where(p => p.comid == transactioncomid && p.AccType == "L" && p.IsCashItem == true).Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text", Vouchermain.AccId);
                this.ViewBag.SubSectionList = db.Cat_SubSection.Where(x => x.ComId == transactioncomid);
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            try
            {
                //GTF_VoucherSub GTF_VoucherSub = db.GTF_VoucherSubs.Find(id);
                //db.GTF_VoucherSubs.Remove(GTF_VoucherSub);

                GTF_VoucherMain Vouchermain = await db.GTF_VoucherMains.FindAsync(id);



                var CurrentVoucherSub = db.GTF_VoucherSubs.Include(x => x.VoucherSubChecnoes).Include(x => x.VoucherSubSections).Where(p => p.VoucherId == Vouchermain.VoucherId);
                db.GTF_VoucherSubs.RemoveRange(CurrentVoucherSub);


                db.GTF_VoucherMains.Remove(Vouchermain);
