...
namespace GTERP.Controllers.Account_GTF
{
    ...
    public class GTF_ChartOfAccountsController : Controller
    {
        ...
        public GTF_ChartOfAccountsController(GTRDBContext _db, TransactionLogRepository tran)
        {
            tranlog = tran;
            db = _db;
            //Repository = rep;
        ...
        public ActionResult Index()
        {
            ...
            List<GTF_ChartOfAccount> chartofaccountlist = db.GTF_ChartOfAccounts.Take(1).Include(x => x.ParentChartOfAccount).Where(c => c.AccId > 0 && c.comid == comid && c.IsSysDefined == 0).ToList();

            return View(chartofaccountlist);
            //modelList = GTF_ChartOfAccount.prcGetData();
        ...
        public ActionResult COATree()
        {
            ...
            //List<GTF_ChartOfAccount> all = new List<GTF_ChartOfAccount>();
            //using (GTRDBContext db = new GTRDBContext())
            //{
            var all = db.GTF_ChartOfAccounts.Where(a => a.comid == comid).OrderBy(a => a.ParentID).ToList();
            //}
            return View(all);
        }
        ...
        public ActionResult Details(int Id)
        {
            ...
            //}
            //prcLoadCombo();
            //return View(GTF_ChartOfAccount.prcGetData(Id));
            return View();
        ...
        public ActionResult Create()
        {
            try
            {
                ...
                string comid = HttpContext.Session.GetString("comid");
                var model = new GTF_ChartOfAccount();


                var lastacccoa = db.GTF_ChartOfAccounts.Where(x => x.comid == comid).OrderByDescending(x => x.AccId).FirstOrDefault();
                ...
                //ViewBag.ParentIdId = new SelectList(db.GTF_ChartOfAccounts, "AccId", "AccName").ToList();

                //GTF_ChartOfAccount model = new GTF_ChartOfAccount();
                //ViewBag.ParentId = new SelectList(db.GTF_ChartOfAccounts, "AccId", "AccName").ToList();


                SqlParameter[] sqlParameter = new SqlParameter[1];
                sqlParameter[0] = new SqlParameter("@comid", comid);

                List<COAtemp> COAParent = Helper.ExecProcMapTList<COAtemp>("GTF_prcgetCOAParent", sqlParameter);
                ...
                //ViewBag.ParentId = db.GTF_ChartOfAccounts.Where(m => m.AccType == "G").ToList();
        ...
        public ActionResult Create(GTF_ChartOfAccount model, string title)
        {
                ...
                //if (ModelState.IsValid)
                {
                    if (model.AccId > 0)
                    {
                        ...
                        if (isModelBase == 1)
                        {
                            ...
                            {
                                var parentcodeinfo = db.GTF_ChartOfAccounts.Where(x => x.AccId == model.ParentID && x.comid == comid).FirstOrDefault();
                                string maxacccsubid = db.GTF_ChartOfAccounts.Where(x => x.ParentID == model.ParentID && x.comid == comid).Max(x => x.AccSubId).ToString();

                                if (model.AccType != model.PrevAccType)
                                {


                                    int ExistAccIdVoucher = db.GTF_VoucherSubs.Where(x => x.AccId == model.AccId).Count();
                                    int ExistAccIdSubAccountsHead = db.GTF_ChartOfAccounts.Where(x => x.ParentID == model.AccId && x.comid == comid).Count();
                                ...
                                if (maxacccsubid == null || maxacccsubid == "")
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        List<COAtemp> COAParent = Helper.ExecProcMapTList<COAtemp>("GTF_prcgetCOAParent", sqlParameter).ToList();
                            ...
                            if (title == "Delete")
                            {
                                ...
                                var count = db.GTF_ChartOfAccounts.Where(m => m.ParentID == model.AccId && m.comid == comid).ToList();
                        ...
                        else
                        {
                            ...
                            ////db.Database.ExecuteSqlCommand("prcTran_GTF_Account  @p0,@p1, @p2, @p3, @p4, @p5, @p6, @p7, @p8, @p9, @p10, @p11, @p12, @p13, @p14, @p15 ", parameters: new[]
                    ...
                    else
                    {
                        ...
                        if (isModelBase == 1)
                        {
                            ...
                            db.GTF_ChartOfAccounts.Add(model);
                            db.SaveChanges();
                    ...
                    //string sqlQuery = "Exec prcTran_GTF_Account 'NEW', " + Session["comid"] + ", 0, '" + model.AccName + "', '" + model.AccType + "', " + model.ParentID + ", " + double.Parse(model.OpDebit.ToString()) + ", " + double.Parse(model.OpCredit.ToString()) + ", 0, 0, 0 , " + model.CountryID + "," + double.Parse(model.Rate.ToString()) + ", " + model.opFYId.ToString() + ", '" + (model.OpDate) + "',0,0";
        ...
        public ActionResult Edit(int? Id)
        {
            ...
            GTF_ChartOfAccount chartofaccount = db.GTF_ChartOfAccounts.Find(Id);
        ...
        // [OverridableAuthorize]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(GTF_ChartOfAccount model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        public JsonResult DeleteConfirmed(int? id)
        {
            try
            {
                ...
                if (isModelBase == 1)
                {
                    ...
                    GTF_ChartOfAccount coa = db.GTF_ChartOfAccounts.Where(c => c.comid == (HttpContext.Session.GetString("comid")) && c.AccId == id).FirstOrDefault();

                    int ExistAccIdVoucher = db.GTF_VoucherSubs.Where(x => x.AccId == coa.AccId).Count();
                    int ExistAccIdSubAccountsHead = db.GTF_ChartOfAccounts.Where(x => x.ParentID == coa.AccId && x.comid == comid).Count();
                    ...
                    {
                        db.GTF_ChartOfAccounts.Remove(coa);
                        db.SaveChanges();
        ...
        public IActionResult Get()
        {
            try
            {
                ...
                var query = from e in db.GTF_ChartOfAccounts.Where(x => x.AccId > 0 && x.IsSysDefined == 0 && x.comid == comid).OrderByDescending(x => x.AccId)
        ...
        public string prcDataSave(GTF_ChartOfAccount model)
        {
        ...
        public string prcUpdateData(GTF_ChartOfAccount model)
        {
            //ArrayList arQuery = new ArrayList();
            //clsProcedure clsProc = new clsProcedure();
            //clsConnectionNew clsCon = new clsConnectionNew(Session["dbACC"].ToString(), true);
            try
            {
                ...
                //sqlQuery = "Update  tblGTF_COA Set Balance=" + dblBalance + ",IsSubsidiaryLedger=" + (((model.isSubsidiaryLedger)== true)? 1 : 0) + " where AccId=" + Int32.Parse(model.accId.ToString()) + " AND comid =  " + Session["comid"] + "";
                //arQuery.Add(sqlQuery);
        ...
        public string prcDeleteData(GTF_ChartOfAccount model)
        {
            //ArrayList arQuery = new ArrayList();
            //clsProcedure clsProc = new clsProcedure();
            //clsConnectionNew clsCon = new clsConnectionNew(Session["dbACC"].ToString(), true);
            try
            {
                ...
                //string sqlQuery = "Exec prcTran_GTF_Account 'DELETE', " + Session["comid"] + ",'" + model.accId + "' , '" + model.AccName + "', '" + ((model.isGroup == true) ? 'G' : 'L') + "', " + model.ParentID + ", " + double.Parse(model.OpDebit.ToString()) + ", " + double.Parse(model.OpCredit.ToString()) + ", 0, 0, 0 , " + model.CountryID + "," + double.Parse(model.Rate.ToString()) + ", " + model.opFYId.ToString() + ", '" + (model.opDate) + "'," + ((model.isSubsidiaryLedger == true) ? 1 : 0) + ",0";
                //arQuery.Add(sqlQuery);

                ////string  sqlQuery = "Exec prcTran_GTF_Account 'DELETE', " + Session["comid"] + ", " + model.accId + ",'" + model.AccCode + "', '', " + model.ParentID + ", 0, 0, 0, 0, 0,0, 0, 0, ''";
                ////arQuery.Add(sqlQuery);
        ...
        public JsonResult AccountsReport(string rptFormat, string action)
        {
            try
            {
                ...
                if (action == "PrintAccCOA")
                {
                    ...
                    var query = "Exec [GTF_rptCOA] '" + comid + "'";
                    HttpContext.Session.SetString("reportquery", "Exec [GTF_rptCOA] '" + comid + "'");
                ...
                //HttpContext.Session.SetObject("GTF_rptList", postData);
