...
namespace GTERP.Controllers.HR
{
    ...
    public class LoanWelfareController : Controller
    {
        private GTRDBContext db;
        public LoanWelfareController(GTRDBContext context)
        {
            db = context;
        }
        public IActionResult Index()
        {
            var loanData = db.HR_Loan_Welfare
        ...
        [HttpPost]
        public IActionResult Create(HR_Loan_Welfare hR_Loan_Welfare, bool newCalculation)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (hR_Loan_Welfare.LoanWelId > 0)
                    {
                        ...
                        db.Entry(hR_Loan_Welfare).State = EntityState.Modified;
                        if (!newCalculation)
                        {
                        ...
                        else
                        {
                            ...
                            var exist = db.HR_Loan_Data_Welfare.Where(l => l.LoanWelId == hR_Loan_Welfare.LoanWelId).ToList();
                    ...
                    {
                        var unPaidLoan = db.HR_Loan_Data_Welfare
                            .Where(l => l.EmpId == hR_Loan_Welfare.EmpId && l.IsPaid == false && l.ComId == comid)
                            .FirstOrDefault();
        ...
        public IActionResult GetEmp(int id)
        {
            ...
            var loandMaster = db.HR_Loan_Welfare
            ...
            if (loandMaster != null)
            {
                ...
                ViewBag.LoanCalculation = db.HR_Loan_Data_Welfare
                    .Where(l => l.LoanWelId == loandMaster.LoanWelId).OrderBy(l => l.DtLoanMonth).ToList();
        ...
        public IActionResult Edit(int id)
        {
            ...
            var loandMaster = db.HR_Loan_Welfare
                ...
                .Include(l => l.HR_Emp_Info.Cat_Designation)
                .Include(l => l.HR_Emp_Info.HR_Emp_Image)
                .Where(l => l.LoanWelId == id).FirstOrDefault();
            string comid = HttpContext.Session.GetString("comid");
            if (loandMaster != null)
            {
                ViewBag.Title = "Edit";
                ...
                ViewBag.LoanCalculation = db.HR_Loan_Data_Welfare
                    .Where(l => l.LoanWelId == id).OrderBy(l => l.DtLoanMonth).ToList();
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            try
            {
                var detailsLoan = await db.HR_Loan_Data_Welfare.Where(l => l.LoanWelId == id).ToListAsync();
                db.HR_Loan_Data_Welfare.RemoveRange(detailsLoan);
                var master = db.HR_Loan_Welfare.Where(l => l.LoanWelId == id).FirstOrDefault();
                db.HR_Loan_Welfare.Remove(master);
