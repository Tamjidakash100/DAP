...
namespace GTERP.Controllers.HR
{
    ...
    public class LoanPFController : Controller
    {
        private GTRDBContext db;
        public LoanPFController(GTRDBContext context)
        {
            db = context;
        }
        public IActionResult Index()
        {
            var loanData = db.HR_Loan_PF
        ...
        [HttpPost]
        public IActionResult Create(HR_Loan_PF hR_Loan_PF, bool newCalculation)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        hR_Loan_PF.UpdateByUserId = comid;
                        hR_Loan_PF.DateUpdated = DateTime.Now.Date;
                        db.Entry(hR_Loan_PF).State = EntityState.Modified;
                        if (!newCalculation)
                        {
                        ...
                        else
                        {
                            ...
                            var exist = db.HR_Loan_Data_PF.Where(l => l.LoanPFId == hR_Loan_PF.LoanPFId).ToList();
                    ...
                    {
                        var unPaidLoan = db.HR_Loan_Data_PF
                            .Where(l => l.EmpId == hR_Loan_PF.EmpId && l.IsPaid == false && l.ComId == comid)
                            .FirstOrDefault();
        ...
        public IActionResult GetEmp(int id)
        {
            ...
            if (loandMaster != null)
            {
                ...
                ViewBag.LoanCalculation = db.HR_Loan_Data_PF
                    .Where(l => l.LoanPFId == loandMaster.LoanPFId).OrderBy(l => l.DtLoanMonth).ToList();
        ...
        public IActionResult Edit(int id)
        {
            ...
            var loandMaster = db.HR_Loan_PF
                ...
                .Include(l => l.HR_Emp_Info.Cat_Designation)
                .Include(l => l.HR_Emp_Info.HR_Emp_Image)
                .Where(l => l.LoanPFId == id).FirstOrDefault();
            string comid = HttpContext.Session.GetString("comid");
            if (loandMaster != null)
            {
                ViewBag.Title = "Edit";
                ...
                ViewBag.LoanCalculation = db.HR_Loan_Data_PF
                    .Where(l => l.LoanPFId == id).OrderBy(l => l.DtLoanMonth).ToList();
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            try
            {
                var detailsLoan = await db.HR_Loan_Data_PF.Where(l => l.LoanPFId == id).ToListAsync();
                db.HR_Loan_Data_PF.RemoveRange(detailsLoan);
                var master = db.HR_Loan_PF.Where(l => l.LoanPFId == id).FirstOrDefault();
                db.HR_Loan_PF.Remove(master);
