...
namespace GTERP.Controllers
{
    ...
    public class SalesController : Controller
    {
        ...
        public SalesController(GTRDBContext context, TransactionLogRepository tran, POSRepository pos)
        {
            tranlog = tran;
            POS = pos;
            db = context;
                ...
                if (y.ToString().Length > 0)
                {
                    ...
                    var query = from e in db.SalesMains//.Where(x => x.comid == comid)
                                ...
                                select new SalesResult
                                {
                                    ...
                                    CustomerName = e.CustomerName != "=N/A=" ? e.CustomerName : e.vCustomertName.CustomerName,
        ...
        public IActionResult POSCreate()
        {
            ...
            try
            {
                ...
                var comid = "F8759147-88B1-4DBA-93B8-E7504B0CE2BE";// HttpContext.Session.GetString("comid");
                var userid = HttpContext.Session.GetString("userid");
        ...
        public async Task<IActionResult> POSEdit(int? id)
        {
            if (id == null)
            {
                return NotFound();
        ...
        public async Task<IActionResult> POSDelete(int? id)
        {
            ...
            SalesMain salesmain =
                ...
                .Include(x => x.SalesTermsSubs)
                .Include(x => x.SalesPaymentSubs).ThenInclude(x => x.vChartofAccounts)
                .Where(x => x.SalesId == id).FirstOrDefault();
        ...
        public async Task<IActionResult> POSPrint(int? id)
        {
            if (id == null)
            {
                return NotFound();
        ...
        public async Task<IActionResult> POSReport(int? id)
        {
            if (id == null)
            {
                return NotFound();
            ...
            var comid = HttpContext.Session.GetString("comid").ToString();
            var userid = HttpContext.Session.GetString("userid").ToString();
            var company = db.Companys.Where(x => x.CompanySecretCode == comid).FirstOrDefault();

            HttpContext.Session.SetString("CompanyName", company.CompanyName);
            HttpContext.Session.SetString("PrimaryAddress", company.PrimaryAddress);
            ...
            //var comid = HttpContext.Session.GetString("comid").ToString();
            //var userid = HttpContext.Session.GetString("userid").ToString();
            //var CompanyName = HttpContext.Session.GetString("CompanyName").ToString();
            //var PrimaryAddress = HttpContext.Session.GetString("PrimaryAddress").ToString();
            //var SecoundaryAddress = HttpContext.Session.GetString("SecoundaryAddress").ToString();
            //var comPhone = HttpContext.Session.GetString("comPhone").ToString();
            //var comWeb = HttpContext.Session.GetString("comWeb").ToString();
            //var comEmail = HttpContext.Session.GetString("comEmail").ToString();
            //var CompanyAddressBangla = HttpContext.Session.GetString("CompanyAddressBangla").ToString();



            SalesMain salesmain = await

                db.SalesMains.Include(x => x.vCustomertName)
                .Include(x => x.SalesSubs).ThenInclude(x => x.vProductName)
                .Include(x => x.SalesSubs).ThenInclude(x => x.vProductSerial)
                .Include(x => x.SalesTermsSubs)
                .Include(x => x.SalesPaymentSubs).ThenInclude(x => x.vChartofAccounts)
        ...
        public JsonResult DeleteConfirmed(int id)
        {
            try
            {
                ...
                SalesMain salesmain = db.SalesMains.Find(id); //.Include(x=>x.SalesSubs)
        ...
        public ActionResult Print(int? id, string type)
        {
            ...
            var reportname = "rptInvoice";
            HttpContext.Session.SetString("ReportPath", "~/ReportViewer/POS/" + reportname + ".rdlc");
            HttpContext.Session.SetString("reportquery", "Exec [rptInvoice] '" + comid + "','" + id + "'");

            string filename = db.SalesMains.Where(x => x.SalesId == id).Single().SalesNo.ToString();
