...
namespace GTERP.Controllers.POS
{
    ...
    public class GoodsReceiveController : Controller
    {
        ...
        public GoodsReceiveController(GTRDBContext context, POSRepository _POS, TransactionLogRepository tran, PermissionLevel _PL)
        {
            db = context;
            tranlog = tran;
            //Repository = rep;
        ...
        public IActionResult DirectGrr()
        {
            ...
            if (lastgoodsreceiveMain != null)
            {
                ...
                ViewData["CurrencyId"] = new SelectList(db.Currency.OrderByDescending(x => x.isDefault), "CurrencyId", "CurCode", goodsreceive.CurrencyId);
        ...
        public IActionResult POSDirectGrr()
        {
        ...
        //        {
        //            goodsReceiveMain.DateUpdated = date;

        //            if (goodsReceiveMain.ComId == null)
        //            {
        //                goodsReceiveMain.ComId = comid;
        //                goodsReceiveMain.UserId = userid;
        //                goodsReceiveMain.DateAdded = date;
        //            }
        //            goodsReceiveMain.DateUpdated = date;
        ...
        //            //if (goodsReceiveMain.GoodsReceiveSub.FirstOrDefault().GRRSubId > 0)
        //            //{
        //                foreach (GoodsReceiveSub item in goodsReceiveMain.GoodsReceiveSub)
        //                {
        //                    item.RemainingReqQty = item.RemainingReqQty + Convert.ToInt32(item.Received);
        //                    if (item.Isdelete==true)
        //                    {
        //                        db.Entry(item).State = EntityState.Deleted;
        //                    }
        //                    else if (item.GRRSubId > 0)
        //                    {
        //                        db.Entry(item).State = EntityState.Modified;
        //                    }
        //                    else
        //                    {
        //                        goodsReceiveMain.GoodsReceiveSub.Add(item);
        //                    }
        ...
        //            {
        //                item.GRRSubId = 0;
        //                item.RemainingReqQty = item.PurchaseQty - Convert.ToInt32(item.Received);

        //            }
        //            //db.Add(goodsReceiveMain);
        //            db.GoodsReceiveMain.Add(goodsReceiveMain);
        ...
        //        ViewData["CurrencyId"] = new SelectList(db.Currency, "CurrencyId", "CurCode", goodsReceiveMain.CurrencyId);
        ...
        //        return Json(new { Success = result, Id = goodsReceiveMain.GRRMainId, ex = "" });
        //    }
        //    catch (Exception ex)
        //    {
        //        return Json(new { Success = "", Id = goodsReceiveMain.GRRMainId, ex = ex.Message });
        //    }
        ...
        public JsonResult PrintMissingSequence(string rptFormat, string action, string Type, string FromNo, string ToNo, int PrdUnitId, string FromDate, string ToDate)
        {
            try
            {
                string comid = HttpContext.Session.GetString("comid");
                ...
                if (action == "PrintMissingSequence")
                {
                    ...
                    var query = "Exec [rpt_MissingSequence] '" + comid + "' ,'" + Type + "' ,'" + FromNo + "' ,'" + ToNo + "' ,'" + PrdUnitId + "'";
